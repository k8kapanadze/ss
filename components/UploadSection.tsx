...
import { parseQuestionFile, formatBytes, isRawEmojiFormat, preprocessRawQuizText } from '../utils';
...
export default function UploadSection({ files, onFilesChanged }: UploadSectionProps) {
  ...
  const handleTextProcessing = (text: string, name: string) => {
    try {
      ...
      // Auto-detect the messy "¢ ID=" / emoji raw format and convert it
      // into the strict //// // /// format before handing it to the
      // platform's native parser.
      if (isRawEmojiFormat(text)) {
        const result = preprocessRawQuizText(text);
        textToParse = result.formatted;
        conversionNote = ` (დამუშავდა ნედლი ფორმატიდან — წარმატებული: ${result.successCount}, შეცდომა: ${result.errorCount})`;

        if (!result.formatted) {
          setErrorMsg(
            `ფაილში "${name}" ნედლი ფორმატის კითხვები ვერ დამუშავდა (${result.errorCount} შეცდომა).`
          );
          setSuccessMsg(null);
          return;
        }
      ...
      const parsed = parseQuestionFile(textToParse, name);
      if (parsed.length === 0) {
      ...
      setSuccessMsg(`ფაილი წარმატებით ჩაიტვირთა — ნაპოვნია ${parsed.length} კითხვა!${conversionNote}`);
      setErrorMsg(null);
    } catch (err) {
